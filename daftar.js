...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.location.hash) {
    ...
    if (hash === 'partner' || hash === 'check' || hash === 'member') {
      activateTab(hash);
    }
  }
  ...
  function syncCardWithCurrentTab() {
    ...
    if (currentTab === 'partner') {
      ...
      if (cardTierText) cardTierText.textContent = 'MITRA BISNIS';
      if (cardTierPill) {
        const crown = cardTierPill.querySelector('.tier-crown');
        if (crown) crown.textContent = '🤝';
      }
      if (cardFlavorLabel) cardFlavorLabel.textContent = 'PAKET KEMITRAAN';
      
      const partnerNameInput = document.getElementById('partnerName');
      const partnerCityInput = document.getElementById('partnerCity');
      const selectedPkgInput = document.getElementById('selectedPackage');

      const partnerName = partnerNameInput ? partnerNameInput.value.trim() : '';
      const partnerCity = partnerCityInput ? partnerCityInput.value.trim() : '';
      const selectedPkg = selectedPkgInput ? selectedPkgInput.value : '';

      const holder = document.getElementById('cardHolderName');
      if (holder) holder.textContent = partnerName ? partnerName.toUpperCase() : 'BUDI PRASETYO, S.E.';
      const cityEl = document.getElementById('cardCityText');
      if (cityEl) cityEl.textContent = partnerCity || 'Semarang Heritage';
      if (cardFlavorText) cardFlavorText.textContent = selectedPkg ? selectedPkg.split(' ')[0] + ' Heritage' : 'Gerobak Heritage';
      if (cardExpiry) cardExpiry.textContent = 'RESMI 3 TAHUN';
      ...
      if (cardTierText) cardTierText.textContent = 'VIP ROYAL';
      if (cardTierPill) {
        const crown = cardTierPill.querySelector('.tier-crown');
        if (crown) crown.textContent = '👑';
      }
      if (cardFlavorLabel) cardFlavorLabel.textContent = 'VARIAN FAVORIT';
      ...
      const memberCity = memberCityInput ? memberCityInput.value : '';
      const selectedFlavor = selectedFlavorInput ? selectedFlavorInput.value : '';

      const holder = document.getElementById('cardHolderName');
      if (holder) holder.textContent = memberName ? memberName.toUpperCase() : 'RADEN MAS DANANG';
      const cityEl = document.getElementById('cardCityText');
      if (cityEl) cityEl.textContent = memberCity ? memberCity.split(' ')[0] : 'Semarang Heritage';
      if (cardFlavorText) cardFlavorText.textContent = selectedFlavor ? selectedFlavor.replace('Lumpia ', '') : 'Rebung Madu Udang';
      if (cardExpiry) cardExpiry.textContent = 'SEUMUR HIDUP';
  ...
  if (memberNameInput) {
    memberNameInput.addEventListener('input', (e) => {
      ...
      if (cardHolderName) cardHolderName.textContent = val ? val.toUpperCase() : 'RADEN MAS DANANG';
    });
  }
  ...
  if (memberCityInput) {
    memberCityInput.addEventListener('change', (e) => {
      ...
      if (cardCityText) cardCityText.textContent = val ? val.split(' ')[0] : 'Semarang Heritage';
      if (window.lumpiaAudio) window.lumpiaAudio.playClick();
    });
  }
  ...
  flavorChips.forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      if (selectedFlavorInput) selectedFlavorInput.value = flavorName;
      if (cardFlavorText) cardFlavorText.textContent = flavorName.replace('Lumpia ', '');
    });
  });
  ...
  if (partnerNameInput) {
    partnerNameInput.addEventListener('input', (e) => {
      if (currentTab === 'partner' && cardHolderName) {
        const val = e.target.value.trim();
  ...
  if (partnerCityInput) {
    partnerCityInput.addEventListener('input', (e) => {
      if (currentTab === 'partner' && cardCityText) {
        const val = e.target.value.trim();
  ...
  packageCards.forEach(pkgCard => {
    pkgCard.addEventListener('click', () => {
      ...
      if (selectedPackageInput) selectedPackageInput.value = pkgName;
      if (currentTab === 'partner' && cardFlavorText) {
  ...
  if (memberRegForm) {
    memberRegForm.addEventListener('submit', (e) => {
      ...
      // Update modal text
      if (modalTitle) modalTitle.textContent = `Sugeng Rawuh, ${name}!`;
      if (modalSubtitle) modalSubtitle.textContent = `Kartu Kanjeng Club VIP resmi milikmu telah diterbitkan dengan Nomor ID: ${memberId}. Diskon 15% langsung aktif!`;
      if (btnWaText) btnWaText.textContent = 'Konfirmasi Voucher ke WhatsApp Resmi Kanjeng';
  ...
  if (partnerRegForm) {
    partnerRegForm.addEventListener('submit', (e) => {
      ...
      localStorage.setItem('kanjeng_current_user', JSON.stringify(partnerObj));
      ...
      if (modalTitle) modalTitle.textContent = `Pengajuan Kemitraan Diterima!`;
      if (modalSubtitle) modalSubtitle.textContent = `Terima kasih ${name}. Kredensial Calon Mitra Anda terdaftar dengan ID: ${partnerId}. Tim Kemitraan Kanjeng akan segera menghubungi Anda.`;
      if (btnWaText) btnWaText.textContent = 'Hubungi Tim Kemitraan Pusat via WhatsApp';
